<%@ Page Title="" Language="C#" MasterPageFile="~/Site1.Master" AutoEventWireup="true" CodeBehind="Default.aspx.cs" Inherits="LuxeComasterpage.index" %>
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">
   
</asp:Content>

<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
       <div class="container2">
           <img src="Bag%20Photos/mcm%20bag%20banner.jpg" alt="Banner" />   
       </div>

    <div class="container4">
   <table>
    <tr><td><img src="Bag%20Photos/tophandlerosso1.jpg" alt="Furla Rosso Top Handle" height="250" width="300"/>

            <div class="productcaption">Furla Rosso Top Handle</div>
            <div class="productprice">BND 400</div>

        </td>

    <td><img src="Bag%20Photos/MetropolisAcciaio1.jpg" alt="Furla Metropolis Accacio" height="250" width="300" />
                    <div class="productcaption">Furla Metropolis Acciaio Crossbody</div>
                    <div class="productprice">BND 320</div>
    </td>
    <td><img src="Bag%20Photos/tonipetrolio1.jpg" alt="Furla Tonipetrolio" height="250" width="300" />
                    <div class="productcaption">Furla Tonipetrolio Crossbody</div>
            <div class="productprice">BND 335</div>
        </td>
    </tr>
    <tr><td> <img src="Bag%20Photos/Furla%20Stacy%20Bucket.jpg" alt="Furla Stacy Bucket Bag" height="250" width="300" />
                   <div class="productcaption">Furla Stacy Bucket Bag</div>
           <div class="productprice">BND 400</div>
        </td>
    <td><img src="Bag%20Photos/Furla%20Crossbody%20Rubino%20A.jpg" alt="Furla Rubino Mini Crossbody" height="250" width="300" />
          <div class="productcaption">Furla Rubino Mini Crossbody</div>
                <div class="productprice">BND 350</div>
    </td>
    <td><img src="Bag%20Photos/furla%20capriccio.jpg" alt="Furla Capriccio" height="250" width="300" />
           <div class="productcaption">Furla Capriccio Satchel Bag</div>
                <div class="productprice">BND 450</div>
    </td>

    </tr>
   <tr><td> <img src="Bag%20Photos/StarkBagpack1.jpg" alt="MCM Stark Backpack" height="300" width="300"/>
         <div class="productcaption">MCM Stark Backpack</div>
                <div class="productprice">BND 1200</div>
       </td>
   <td> <img src="Bag%20Photos/StarkBeeboo1.jpg" alt="MCM Stark BeeBoo" height="300" width="300"/>
                  <div class="productcaption">MCM Stark BeeBoo X-MINI</div>
                <div class="productprice">BND 1080</div>
   </td>
    <td><img src="Bag%20Photos/Duchesspolkestuds1.jpg" alt="MCM Duchess Polke" height="300" width="300" />
                        <div class="productcaption">MCM Duchess Polke Studs</div>
                <div class="productprice">BND 1210</div>

    </td></tr>
      </table>        
        <h2> View our BAGS page for more details... </h2>
    </div>

    <div class="container5">
        <img src="Bag%20Photos/furlabanner.jpg" alt="Banner" />
    </div>

    <a href="https://validator.w3.org/nu/?doc=http%3A%2F%2F1526897.studentwebserver.co.uk%2Fco5027%2FDefault.aspx">HTML Validation</a>
    
</asp:Content>


